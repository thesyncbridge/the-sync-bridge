...
import { Triangle, Hexagon, Activity, Zap, Globe, ChevronRight, Clock, Users, Award, Play, ExternalLink, Mail, Check, Menu, X, ShoppingBag, Trash2, Plus, Lock, Package, Shirt, MessageCircle, Reply, Send, Upload, Image } from "lucide-react";
...
const MemberGear = () => {
  ...
  const addToCart = (type, size = null) => {
    ...
    if (existing) {
      ...
      setCart([...cart, { type, size, quantity: 1, price: gear[type].price }]);
    }
    toast.success(`Added ${gear[type].name} to cart`);
  };
